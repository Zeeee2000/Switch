...
describe('Login Testcases', () => {
  ...
  it('Verify the 5 digit email verification code is asked upon logging in', () => {
    ...
    cy.fixture('Login.json').then((loginData) => {
      ...
      ln.ClickSignIn();
      cy.get('.sc-f5ac156-3').contains('Enter Email Code');
    });
  });
...
// it('Verify that the message displays, that the email has already been verified if the link is clicked again (Failed)', () => {
//   const ln = new LoginPage();
//   cy.fixture('Login.json').then((loginData) => {
//     const { Signup } = loginData;
//         let id= "tegspcsa";
...
// it('Verify the 2fa modal will be closed on clicking the cancel button', function () {
...
//       ln.ClickSignIn();
//       let id= "tegspcsa";
//       ln.AssertCancelAndLoginUrl();
      
//     });   
//   });
// });

//   it('Verify that the system properly logs the user out or prompts for reauthentication.', () => {
//     const ln = new LoginPage();
//     cy.fixture('Login.json').then((loginData) => {
//       const { validUser } = loginData;
//       ln.FillEmail(validUser.username);
